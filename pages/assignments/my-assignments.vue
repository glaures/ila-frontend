...
async function loadOverview() {
  ...
  errorMsg.value = null
  try {
    finalized.value = await $authFetch('/assignment-results/is-finalized')
...
<template>
  <div class="container py-4">
    ...
    <template v-else>
      <!-- Zuweisung noch nicht final -->
      <div v-if="!finalized" class="alert alert-info d-flex align-items-center" role="alert">
        <i class="bi bi-info-circle-fill me-2 fs-5"></i>
        <div>
          Die Zuweisung der Kurse ist noch nicht abgeschlossen. Sobald die endgültige Einteilung
          feststeht, werden deine Kurse hier angezeigt.
        </div>
      </div>

      <!-- Zuweisung final – Kurse anzeigen -->
